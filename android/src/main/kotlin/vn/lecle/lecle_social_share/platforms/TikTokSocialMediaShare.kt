...
//                                )
    ...
    fun shareFilesToTikTokTemp(
        ...
        fileUrls?.let {
            val hasPackage = StoreService.isPackageInstalled(context, TIKTOK_T_PACKAGE) || StoreService.isPackageInstalled(context, TIKTOK_M_PACKAGE)
            if (!hasPackage) {
                flutterResult.success(false)
                StoreService.openAppOnStore(context, TIKTOK_T_PACKAGE)
            } else {
...
//                    request.callerLocalEntry = context.packageName + ".aimirror003.MainActivity"
                    callbackActivity?.let {
                        request.callerLocalEntry = context.packageName + it
                    }
    ...
    fun tiktokInstalled(context: Activity): Boolean {
        return StoreService.isPackageInstalled(context, TIKTOK_T_PACKAGE) || StoreService.isPackageInstalled(context, TIKTOK_M_PACKAGE)
    }
}